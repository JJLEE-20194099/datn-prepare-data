...
meeyland_df = pd.read_csv('./files/private_house_meey_land_location.csv')
...
fig1 = px.scatter_mapbox(location_df, lat='lat', lon="lon", text = location_df['alley_price'].to_numpy(), color="street_price", color_continuous_scale=[[0, 'green'], [0.5, 'red'], [1.0, 'rgb(0, 0, 255)']],
                                zoom=8, size = "street_price", size_max=20, opacity=1, width=880)
...
fig2 = px.scatter_mapbox(meeyland_df, lat='lat', lon="lon", color="price", color_continuous_scale=[[0, 'green'], [0.5, 'red'], [1.0, 'rgb(0, 0, 255)']],
                                zoom=8, size = "price", size_max=20, opacity=1, width=880)
...
fig3 = go.Figure(data=[
    go.Bar(name='TownHouse Price', x=district_list, y=street_price),
    go.Bar(name='Alley 1(fitThreeCars) - House Price', x=district_list, y=alley_1_price),
    go.Bar(name='Alley 2(fitOneCarAndMotorbike) - House Price', x=district_list, y=alley_2_price),
    go.Bar(name='Alley 3(narrowRoad) - House Price', x=district_list, y=alley_3_price)
])
fig3.update_layout(barmode='group', width = 1500)
...
def display_demographic_statistics(selected_district):
    ...
    return {
            'data': [
                go.Bar(name='TownHouse Price', x=ward_list, y=street_price),
                go.Bar(name='Alley 1(fitThreeCars) - House Price', x=ward_list, y=alley_1_price),
                go.Bar(name='Alley 2(fitOneCarAndMotorbike) - House Price', x=ward_list, y=alley_2_price),
                go.Bar(name='Alley 3(narrowRoad) - House Price', x=ward_list, y=alley_3_price)
            ]
    }
...
def display_demographic_distribution(selected_district):
    ...
    trace0 = go.Histogram(name='Alley 1(fitThreeCars) - House Price', x=[street["estimatePrice"]["output"]["alleyHousePrice"]["1"]["mean"] for street in street_by_district])
    trace1 = go.Histogram(name='Alley 2(fitOneCarAndMotorbike) - House Price',x=[street["estimatePrice"]["output"]["alleyHousePrice"]["2"]["mean"] for street in street_by_district])
    trace2 = go.Histogram(name='Alley 3(narrowRoad) - House Price',x=[street["estimatePrice"]["output"]["alleyHousePrice"]["3"]["mean"] for street in street_by_district])
    trace3 = go.Histogram(name='TownHouse Price',x=[street["estimatePrice"]["output"]["streetHousePrice"]["mean"] for street in street_by_district])
...
html_page = html.Div([
    ...
    html.Div([
        html.Div([
            html.Div([html.H1('Ha Noi - BKPrice Map')], style={'textAlign': 'center'}),
            dcc.Graph(figure=fig1)
        ], style={"flex": 1, "margin-right": 12}),

        html.Div([
            html.Div([html.H1('Ha Noi - MeeyMapPrice Map')], style={'textAlign': 'center'}),
