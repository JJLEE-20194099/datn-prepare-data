...
fig3 = go.Figure(data=[
    ...
    go.Bar(name='Alley 3(narrorRoad) - House Price', x=district_list, y=alley_3_price)
])
...
app = Dash(name=__name__, external_stylesheets=[dbc.themes.MINTY, dbc.icons.FONT_AWESOME], suppress_callback_exceptions=True)
...
def display_demographic_statistics(selected_district):
    ...
    return {
            'data': [
                ...
                go.Bar(name='Alley 3(narrorRoad) - House Price', x=ward_list, y=alley_3_price)
            ]
    }
...
def display_demographic_distribution(selected_district):
    ...
    trace2 = go.Histogram(name='Alley 3(narrorRoad) - House Price',x=[street["estimatePrice"]["output"]["alleyHousePrice"]["3"]["mean"] for street in street_by_district])
